function [EEG] = myASR(EEG,fileList,varargin)
...
%## SET DEFAULTS
burstCritASR = p.Results.burstCritASR;
useExternalCalibASR = p.Results.useExternalCalibASR;
...
if useExternalCalibASR && ~isempty(fileList(restIndex)) 
    cleanEEGforCalib_noChRej = ALLEEG(restIndex); %ROEHL! need to make this smarter in the future. This is not dummy proof
