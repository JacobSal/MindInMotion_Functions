function [signal,sample_mask] = clean_windows_CL(signal,max_bad_channels,zthresholds,window_len,window_overlap,max_dropout_fraction,min_clean_fraction,truncate_quant,step_sizes,shape_range)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Editted by Chang Liu: 20220819
% Sort the window rejection criteria and only remove max 10% data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% handle inputs
...
%------------- Add constraint to remove max 10% data ----------------
if mean(sample_mask) < 0.9
    % adopt an iterative process so that max10% data removed
    while mean(sample_mask) < 0.9
        % if not within max 10% data, increase zthresholds
        zthresholds(2) = zthresholds(2) + 1;
        % determine which windows to remove
        remove_mask = false(1,size(swz,2));
        if max(zthresholds)>0
            remove_mask(swz(end-max_bad_channels,:) > max(zthresholds)) = true; end
        ...
        sample_mask = true(1,S); 
        sample_mask(removed_samples(:)) = false;
        fprintf('Recompute threshold');
        fprintf('Keeping %.1f%% (%.0f seconds) of the data.\n',100*(mean(sample_mask)),nnz(sample_mask)/signal.srate);
    end
end
disp(zthresholds);
signal.etc.window_zthreshold = zthresholds;
%--------------------------------------------------------------------
