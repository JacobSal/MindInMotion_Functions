...
%     IC_powpow = find(IC_all_brain >= 5);
    if ~isempty(powpow_ics)
        ...
        powpow_ppicrej = struct('do_plot',params_pp.do_plot, ...
            'save_dir',REJ_STRUCT.plot_save_dir, ...
            'ic_cutoff',params_pp.ic_cutoff, ...
            'low_freq_cutoff',params_pp.low_freq_cutoff, ...
            'high_freq_cutoff',params_pp.high_freq_cutoff);
        bad_ics_out = PowPowCat_ICrej(tmp, ...
            'REJ_STRUCT',powpow_ppicrej);
        %## POP_PROPS_EXTENDED
...
if REJ_STRUCT.do_valid_plots
    ...
    cmap_in = linspecer(length(bad_psd_ics));
    figure('color','w');
    hold on;
    ...
    hold off;
    title('BAD: IC Activity Power Spectrum', ...
        'units','normalized', ...
        'fontsize', 14, ...
        'FontWeight', 'Normal');
    ylabel('Power 10*log_{10}(uV^2/Hz)', 'fontsize', 14); 
    ...
    %## (PLOT 7) BEMOBIL PIPELINE PLOT (TOPOS?)
    ...
    bemobil_plot_patterns_CL(EEG.icawinv,EEG.chanlocs, ...
        'chan_to_plot',find(all_brain_ics >= 8)', ...
        'weights',summed_scores_to_keep,...
        'fixscale',0, ...
        'minweight',0.75, ...
        'titles',titles_FEM);
    fig_i = get(groot,'CurrentFigure');
