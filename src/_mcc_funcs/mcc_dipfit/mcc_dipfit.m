function [error_code] = mcc_dipfit(working_dir,eeg_fpath,source_out_fpath,varargin)
...
if ~exist([working_dir filesep 'vol.mat'],'file') || ~exist([working_dir filesep 'elec_aligned.mat'],'file') || FORCE_RECREATE 
    ...
    [vol,elec_aligned] = fem_create_vol(working_dir,...
        ...
        VOL_CONDUCTIVITIES);
    endi = toc(starti);
    fprintf('Call to fem_create_vol() took %0.2g seconds\n',endi);
else
    %- load vol.mat
...
if ~exist([working_dir filesep 'headmodel_fem_tr.mat'],'file') || FORCE_RECREATE
    ...
    endi = toc(starti);
    fprintf('Call to ft_prepare_leadfield.m took %0.2g seconds\n',endi);
else
...
% EEG        = load('-mat', [fPath filesep fName]);
% EEG        = EEG.EEG;
% fid        = fopen([fPath filesep EEG.data], 'r', 'ieee-le');
