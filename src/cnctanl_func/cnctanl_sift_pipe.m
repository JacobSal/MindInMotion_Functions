...
%           `-    \`_`"'-
% Code Designer: Jacob Salminen
% Code Date: 12/30/2022, MATLAB 2023b
...
for cond_i = 1:length(ALLEEG)
    ...
    tvalid = ALLEEG(cond_i).CAT.VALIDATION;
    POSS_WHITE_STATS = {'acf','ljungbox','boxpierce','limcleod'};
    ws_fn = fieldnames(tvalid.whitestats);
    ws_fn = find(cellfun(@(x) any(strcmp(x,ws_fn)),POSS_WHITE_STATS));
    for ws = 1:length(ws_fn)
        tmp = tvalid.whitestats.(POSS_WHITE_STATS{ws}).w;
        if ~all(tmp)
            tmp = sum(tmp)/length(tmp);
            fprintf(1,['%s) WARNING: Residuals are not completely white!', ...
                'Model fit may not be valid.\n', ...
                '\tPercent White: %0.2f\n'],POSS_WHITE_STATS{ws},tmp);
        end
    end
    fprintf('Mean PC: %0.2f. This should be greater than 85%.\n',mean(tvalid.PCstats.PC))
    fprintf('Mean Stability: %0.2f. This should be close to 1.\n',mean(tvalid.stabilitystats.stability))
    % fprintf('Residual stats: %0.2f. This should be close to 1.\n',mean(tvalid.residualstats.autocorrelation))
end
