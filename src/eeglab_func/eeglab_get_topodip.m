...
if ~isempty(ALLEEG)
    if ~chk || CALC_STRUCT.recalculate
        ...
        tmp_study = STUDY;
        if isfield(tmp_study.cluster,'topox') || ...
                isfield(tmp_study.cluster,'topoall') ||...
        ...
        for j = 1:length(CALC_STRUCT.cluster_inds) % For each cluster requested
            ...
            if ~isempty(STUDY.cluster(cl_i).sets)
                tmp_study = std_readtopoclust_CL(STUDY,ALLEEG,cl_i);% Using this custom modified code to allow taking average within participant for each cluster
