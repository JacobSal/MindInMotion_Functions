...
function [STUDY,h] = std_topoplot_CL(STUDY,cls,mode)
...
h_topo = figure;
set(h_topo,'Units','inches','Position',FIGURE_POSITION)
set(h_topo,'PaperUnits','inches','PaperSize',[1 1],'PaperPosition',[0 0 1 1])
...
%     figure
    ...
    if len ~= 1
        ...
        set(h_topo,'name', maintitle);
    else
        title([ STUDY.cluster(cls(k)).name ' (' num2str(length(unique(STUDY.cluster(cls(k)).sets(1,:)))) ' Ss, ' num2str(length(STUDY.cluster(cls(k)).comps)),' ICs)']);
        set(h_topo,'name',['Scalp map of ' STUDY.cluster(cls(k)).name ' (' num2str(length(unique(STUDY.cluster(cls(k)).sets(1,:)))) ' Ss, ' num2str(length(STUDY.cluster(cls(k)).comps)),' ICs)']);
