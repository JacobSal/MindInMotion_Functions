...
set(gca,'box','off','xtick',[],'ytick',[],...
        'ztick',[],'xcolor',[0,0,0],'ycolor',[0,0,0]);
...
%         tmp.component = comps_i(ind);
    ...
    switch PLOT_TYPE
        case 'average_nogroup'
            ...
            DIPPLOT_STRUCT.dipolesize =  5.8376*(20*3)*3*2; 
            % dipplot(avgdip,DIPPLOT_STRUCT);
            scatter3_dipplot(avgdip,DIPPLOT_STRUCT);
            % cnt = 1; %length(fig.Children(end).Children);
            % for ii = 1:length(dips)-1
            %     if cnt > 3
            %         fig.Children(end).Children(cnt-1).Marker = 'o';
            %         fig.Children(end).Children(cnt-1).ColorVariable = [];
            %         fig.Children(end).Children(cnt-1).CData = [0,0,0];
            %         fig.Children(end).Children(cnt-1).LineWidth = fig.Children(end).Children(cnt-1).SizeData*0.04;
            %         fig.Children(end).Children(cnt-1).MarkerFaceColor = DIPPLOT_STRUCT.color{1}; %fig.Children(end).Children(cnt-1+1).Color;
            %         % fig.Children(end).Children(cnt-1).MarkerFaceAlpha = 0.75;
            %         fig.Children(end).Children(cnt-1).SizeData = fig.Children(end).Children(cnt-1).SizeData*1; %fig.Children(end).Children(cnt+3+1).MarkerSize*2.6;
            %     end
            %     cnt = cnt + 2;
            % end
        case 'all_nogroup'
        ...
        case 'all_group'
            ...
            for ii = 1:length(dips)
                ...
                % scatter3_dipplot(dips(ii),DIPPLOT_STRUCT);
                %(10/29/2024) JS, this uses scatter3() ((NOW)) to display the marker,
                %so use that as a basis for property changes
                % fig.Children(end).Children(cnt+3).Marker = GROUP_MARKERS{gg};
                % fig.Children(end).Children(cnt+3).ColorVariable = [];
                % fig.Children(end).Children(cnt+3).CData = [0,0,0];
                % fig.Children(end).Children(cnt+3).LineWidth = fig.Children(end).Children(cnt+3).SizeData*0.04;
                % fig.Children(end).Children(cnt+3).MarkerFaceColor = fig.Children(end).Children(cnt+3+1).Color;
                % fig.Children(end).Children(cnt+3).MarkerFaceAlpha = 0.666;
                % fig.Children(end).Children(cnt+3).SizeData = fig.Children(end).Children(cnt+3).SizeData*4; %fig.Children(end).Children(cnt+3+1).MarkerSize*2.6;
                %-
                if cnt > 3
                    fig.Children(end).Children(cnt).Marker = GROUP_MARKERS{gg};
                    fig.Children(end).Children(cnt).ColorVariable = [];
                    fig.Children(end).Children(cnt).CData = [0,0,0];
                    fig.Children(end).Children(cnt).LineWidth = fig.Children(end).Children(cnt).SizeData*0.04;
                    fig.Children(end).Children(cnt).MarkerFaceColor = DIPPLOT_STRUCT.color{1}; %fig.Children(end).Children(cnt-1+1).Color;
                    fig.Children(end).Children(cnt).MarkerFaceAlpha = 0.75;
                    fig.Children(end).Children(cnt).SizeData = fig.Children(end).Children(cnt).SizeData*4; %fig.Children(end).Children(cnt+3+1).MarkerSize*2.6;
                    cnt = cnt - 1;
                end
                %(10/29/2024) JS, this uses plot3() to display the marker,
                %so use that as a basis for property changes
                % fig.Children(end).Children(cnt+3).Marker = GROUP_MARKERS{gg};
                ...
                % fig.Children(end).Children(cnt+3).MarkerSize = fig.Children(end).Children(cnt+3+1).MarkerSize*2.6;
                % %-
                % if cnt > 3
                %     fig.Children(end).Children(cnt-1).Marker = GROUP_MARKERS{gg};
                %     fig.Children(end).Children(cnt-1).LineStyle = '-';
                %     fig.Children(end).Children(cnt-1).Color = [0,0,0];
                %     fig.Children(end).Children(cnt-1).LineWidth = fig.Children(end).Children(cnt).MarkerSize*0.05;
                %     fig.Children(end).Children(cnt-1).MarkerFaceColor = DIPPLOT_STRUCT.color{1}; %fig.Children(end).Children(cnt-1).Color;
                %     % fig.Children(end).Children(cnt+3).MarkerSize = fig.Children(end).Children(cnt+3+1).MarkerSize*2.5;
                %     fig.Children(end).Children(cnt-1).MarkerSize = fig.Children(end).Children(cnt).MarkerSize*2.6;
                %     cnt = cnt - 2;
                % end
            end
